...
let test_area_scene_returns_nodes () =
  let nodes =
    Area.scene ~series:[ series_a ] ~x:fst ~width:400.0 ~height:300.0 ()
  in
  Alcotest.(check bool) "non-empty scene" true (List.length nodes > 0)

let test_area_scene_empty_data () =
  let empty_s = mk_series ~label:"empty" ~color:Color.categorical.(0) [] in
  let nodes =
    Area.scene ~series:[ empty_s ] ~x:fst ~width:400.0 ~height:300.0 ()
  in
  Alcotest.(check int) "empty scene" 0 (List.length nodes)

let test_area_scene_matches_view () =
  let scene_nodes =
    Area.scene ~series:[ series_a; series_b ] ~x:fst ~width:400.0 ~height:300.0
      ()
  in
  let view_el =
    Area.view ~series:[ series_a; series_b ] ~x:fst ~width:400.0 ~height:300.0
      ()
  in
...
let () =
  Alcotest.run "Area.scene"
    [
      ( "scene",
        [
          Alcotest.test_case "returns_nodes" `Quick
            test_area_scene_returns_nodes;
          Alcotest.test_case "empty_data" `Quick test_area_scene_empty_data;
          Alcotest.test_case "matches_view" `Quick test_area_scene_matches_view;
        ] );
    ]
