...
let set_title title f =
  ...
  Fut.await fut (function
    | Ok () -> f ()
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.set_title failed"; err ]))

let set_fullscreen flag f =
  let fut =
    Fut.of_promise
      ~ok:(fun _ -> ())
...
let minimize f =
  let fut = Fut.of_promise ~ok:(fun _ -> ()) (invoke_window "minimize" [||]) in
  Fut.await fut (function
    | Ok () -> f ()
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.minimize failed"; err ]))

let maximize f =
  let fut = Fut.of_promise ~ok:(fun _ -> ()) (invoke_window "maximize" [||]) in
  Fut.await fut (function
    | Ok () -> f ()
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.maximize failed"; err ]))
...
let unmaximize f =
  ...
  Fut.await fut (function
    | Ok () -> f ()
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.unmaximize failed"; err ]))

let is_maximized f =
  let fut = Fut.of_promise ~ok:Jv.to_bool (invoke_window "is_maximized" [||]) in
  Fut.await fut (function
    | Ok v -> f v
    | Error err ->
        Brr.Console.(
...
let set_size size f =
  ...
  Fut.await fut (function
    | Ok () -> f ()
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.set_size failed"; err ]))
...
let inner_size f =
  ...
  Fut.await fut (function
    | Ok jv ->
        ...
        f { width; height }
    | Error err ->
        Brr.Console.(error [ str "nopal_tauri: Window.inner_size failed"; err ]))
