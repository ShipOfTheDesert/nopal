let () =
  ...
  let module R = Nopal_runtime.Runtime.Make (
    Virtual_list_bench :
      Nopal_mvu.App.S
        with type model = Virtual_list_bench.model
         and type msg = Virtual_list_bench.msg) in
  ...
  let w = Jv.get Jv.global "window" in
  ignore (Jv.call w "requestAnimationFrame" [| Jv.repr !raf_loop |]);
  (* Expose dispatch for bench scroll tests *)
  let dispatch_js =
    Jv.callback ~arity:1 (fun (msg_name : Jv.t) ->
        let name = Jv.to_string msg_name in
        let msg : Virtual_list_bench.msg option =
          match name with
          | s when String.length s > 7 && String.sub s 0 7 = "scroll:" -> (
              let offset_str = String.sub s 7 (String.length s - 7) in
              match float_of_string_opt offset_str with
              | Some offset -> Some (Scrolled offset)
              | None -> None)
          | _ -> None
        in
        match msg with
