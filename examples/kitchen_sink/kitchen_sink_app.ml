...
module Make (Platform : Nopal_platform.Platform.S) = struct
  module Storage = (val Platform.storage)

  (* Typed wrapper over the same backend (REQ-F4): persists an [int] through
     encode/decode rather than raw strings. The storage section's "Typed
     counter" controls demonstrate it, including the [Decode] error path. *)
  module Codec = Nopal_storage.With_codec (Storage)
  ...
  type codec_state =
    ...
    | CodecLoadedNone
    | CodecCorrupted
    | CodecDecodeError of string
        (** [With_codec] surfaced a [Decode] failure *)
    | CodecBackendError of string
  ...
  type msg =
    ...
    | StorageSet
    | StorageSetResult of (unit, Nopal_storage.error) result
    | StorageGet
    | StorageGetResult of (string option, Nopal_storage.error) result
    | StorageDelete
    | StorageDeleteResult of (unit, Nopal_storage.error) result
    | StorageList
    | StorageListResult of (string list, Nopal_storage.error) result
    | StorageClear
    | StorageClearResult of (unit, Nopal_storage.error) result
    | StorageReload
    | CodecIncrement
    | CodecSave
    | CodecSaveResult of (unit, Nopal_storage.error) result
    | CodecLoad
    | CodecLoadResult of (int option, Codec.typed_error) result
    | CodecCorrupt
    | CodecCorruptResult of (unit, Nopal_storage.error) result
  ...
  let update model = function
    ...
    | SubCounterMsg sub_msg ->
        let sub_counter, sub_cmd =
          Sub_counter.update model.sub_counter sub_msg
        in
    ...
    | Data_table_msg dt_msg ->
        let data_table, dt_cmd =
          Sub_data_table.update model.data_table dt_msg
        in
    ...
    | DrawPointerLeave ->
        ({ model with draw_pointer = None }, Nopal_mvu.Cmd.none)
    ...
    | PanePointerMove x -> (
        ...
        | Some last_x ->
            ...
            let scale =
              Domain_window.width model.pane_domain_window /. chart_w
            in
    ...
    | LinePointerMove x -> (
        ...
        | Some last_x ->
            ...
            let px_delta = x -. last_x in
            let scale =
              Domain_window.width model.line_domain_window /. chart_w
            in
            let delta = -.(px_delta *. scale) in
            let dw =
    ...
    | FetchClicked ->
        ( { model with http_state = Loading },
          Nopal_http.get "https://jsonplaceholder.typicode.com/todos/1"
            (fun o -> FetchResult o) )
    ...
    | GotAppName name ->
        ({ model with tauri_app_name = name }, Nopal_mvu.Cmd.none)
    ...
    | StorageSet ->
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> StorageSetResult r)
               (Storage.set ~key:model.storage_key ~value:model.storage_value))
        )
    | StorageSetResult (Ok ()) ->
        ({ model with storage_state = StorageOk "Stored" }, Nopal_mvu.Cmd.none)
    | StorageSetResult (Error e) ->
    ...
    | StorageGet ->
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> StorageGetResult r)
               (Storage.get model.storage_key)) )
    | StorageGetResult (Ok (Some v)) ->
        ({ model with storage_state = StorageValue v }, Nopal_mvu.Cmd.none)
    | StorageGetResult (Ok None) ->
        ({ model with storage_state = StorageNotFound }, Nopal_mvu.Cmd.none)
    | StorageGetResult (Error e) ->
        ( { model with storage_state = StorageError (Nopal_storage.message e) },
          Nopal_mvu.Cmd.none )
    | StorageDelete ->
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> StorageDeleteResult r)
               (Storage.delete model.storage_key)) )
    | StorageDeleteResult (Ok ()) ->
        ({ model with storage_state = StorageOk "Deleted" }, Nopal_mvu.Cmd.none)
    | StorageDeleteResult (Error e) ->
        ( { model with storage_state = StorageError (Nopal_storage.message e) },
          Nopal_mvu.Cmd.none )
    | StorageList ->
        let keys_task =
          match model.storage_key with
          | "" -> Storage.keys ()
          | prefix -> Storage.keys ~prefix ()
        in
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map (fun r -> StorageListResult r) keys_task) )
    | StorageListResult (Ok keys) ->
        ({ model with storage_state = StorageKeys keys }, Nopal_mvu.Cmd.none)
    | StorageListResult (Error e) ->
        ( { model with storage_state = StorageError (Nopal_storage.message e) },
          Nopal_mvu.Cmd.none )
    | StorageClear ->
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> StorageClearResult r)
               (Storage.clear ())) )
    | StorageClearResult (Ok ()) ->
        ({ model with storage_state = StorageOk "Cleared" }, Nopal_mvu.Cmd.none)
    | StorageClearResult (Error e) ->
        ( { model with storage_state = StorageError (Nopal_storage.message e) },
          Nopal_mvu.Cmd.none )
    | StorageReload -> (model, Nopal_mvu.Cmd.none)
    | CodecIncrement ->
        ({ model with codec_count = model.codec_count + 1 }, Nopal_mvu.Cmd.none)
    | CodecSave ->
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> CodecSaveResult r)
               (Codec.set ~key:codec_count_key ~value:model.codec_count
                  ~encode:string_of_int)) )
    | CodecSaveResult (Ok ()) ->
        ({ model with codec_state = CodecSaved }, Nopal_mvu.Cmd.none)
    | CodecSaveResult (Error e) ->
        ( {
            model with
            codec_state = CodecBackendError (Nopal_storage.message e);
          },
          Nopal_mvu.Cmd.none )
    | CodecLoad ->
        let decode s =
        ...
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> CodecLoadResult r)
               (Codec.get codec_count_key ~decode)) )
    | CodecLoadResult (Ok (Some n)) ->
        ( { model with codec_count = n; codec_state = CodecLoaded n },
          Nopal_mvu.Cmd.none )
    | CodecLoadResult (Ok None) ->
        ({ model with codec_state = CodecLoadedNone }, Nopal_mvu.Cmd.none)
    | CodecLoadResult (Error (Codec.Decode msg)) ->
        ({ model with codec_state = CodecDecodeError msg }, Nopal_mvu.Cmd.none)
    | CodecLoadResult (Error (Codec.Storage e)) ->
        ( {
            model with
            codec_state = CodecBackendError (Nopal_storage.message e);
          },
          Nopal_mvu.Cmd.none )
    | CodecCorrupt ->
        (* Write a non-integer under the typed key via the raw backend so the
           next Load exercises [With_codec]'s [Decode] error path. *)
        ( model,
          Nopal_mvu.Cmd.task
            (Nopal_mvu.Task.map
               (fun r -> CodecCorruptResult r)
               (Storage.set ~key:codec_count_key ~value:"not-a-number")) )
    | CodecCorruptResult (Ok ()) ->
        ({ model with codec_state = CodecCorrupted }, Nopal_mvu.Cmd.none)
    | CodecCorruptResult (Error e) ->
        ( {
            model with
            codec_state = CodecBackendError (Nopal_storage.message e);
          },
    ...
    | TauriStoreSetResult (Error e) ->
        ( { model with tauri_store_state = TauriStoreError e },
          Nopal_mvu.Cmd.none )
    | TauriStoreGet -> (model, Nopal_mvu.Cmd.none)
    | TauriStoreGetResult (Ok (Some v)) ->
        ( { model with tauri_store_state = TauriStoreValue v },
          Nopal_mvu.Cmd.none )
    | TauriStoreGetResult (Ok None) ->
        ( { model with tauri_store_state = TauriStoreNotFound },
          Nopal_mvu.Cmd.none )
            ...
            Element.box ~style:fixed_width_style
              ...
              [
                Element.styled_text
                  ...
                  "This text is long enough to be truncated with an ellipsis \
              ...
              [
                Element.text
                  "Justified text that needs to be long enough to show the \
                   justification effect across the full width of the \
                   container.";
              ];
  ...
  let view_interaction_states model =
    ...
    view_section "Interaction States"
      [
        ...
        Element.button ~style:btn_base_style
          ~interaction:pressed_btn_interaction
  ...
  let view_draw model =
    ...
    let stroke_scene =
      [
        ...
        Scene.line
          ~stroke:
            (Paint.stroke ~width:3.0 ~dash:[ 8.0; 4.0 ] (Paint.solid blue))
          ~x1:10.0 ~y1:35.0 ~x2:100.0 ~y2:35.0 ();
        (* Round cap *)
        Scene.line
          ~stroke:
            (Paint.stroke ~width:6.0 ~line_cap:Round_cap (Paint.solid red))
    ...
    let text_scene =
      [
        ...
        Scene.text ~x:10.0 ~y:40.0 ~font_size:12.0
          ~font_weight:Nopal_style.Font.Bold ~fill:(Paint.solid black)
          "Bold text";
    ...
    let transform_scene =
      [
        ...
        Scene.group
          ~transforms:
            [
              Transform.translate ~dx:140.0 ~dy:25.0;
  ...
  let view_chart_extensions model =
    ...
    let volume_data =
      List.map (fun (x, _, _, _, _, vol) -> (x, vol)) ohlc_data
    in
      ...
      [
        ...
        Element.column ~style:section_body_style
          [
            ...
            Element.text
              "Multi-pane layout (candlestick + volume + drawdown, \
               synchronized):";
            Element.box
              ...
              [
                Chart_pane.view
                  ~panes:
                    [
                      Chart_pane.pane ~height_ratio:0.6
                        (fun dw ~width ~height ->
                          Trading.Candlestick.view ~data:ohlc_data
                            ~x:(fun (x, _, _, _, _, _) -> x)
                            ~open_:(fun (_, o, _, _, _, _) -> o)
                      ...
                      Chart_pane.pane ~height_ratio:0.2
                        (fun dw ~width ~height ->
            ...
            Element.text
              "Line chart with pan/zoom (1,000 data points \xe2\x80\x94 drag \
               to pan, scroll to zoom, or use +/- buttons):";
            Element.row ~style:row_style
              [
                Element.box
                  ~style:
                  ...
                  [
                    Line.view
                      ~series:
                        [
                          Line.series ~smooth:true ~label:"Signal"
                            ~color:cat.(1)
                            ~y:(fun (_, y) -> y)
                            panzoom_data;
                        ]
  ...
  let view_tauri_window model =
    ...
    let fullscreen_label =
      if model.tauri_is_fullscreen then "Exit Fullscreen"
  ...
  let view_storage model =
    ...
    view_section
      ...
      [
        ...
        Element.row ~style:button_row_style
          [
            ...
            Element.button ~on_click:StorageGet
              ...
              (Element.text "Get");
            Element.button ~on_click:StorageDelete
              ~attrs:[ ("data-testid", "storage-delete-btn") ]
              (Element.text "Delete");
            Element.button ~on_click:StorageList
              ~attrs:[ ("data-testid", "storage-list-btn") ]
              (Element.text "List");
            Element.button ~on_click:StorageClear
              ~attrs:[ ("data-testid", "storage-clear-btn") ]
              (Element.text "Clear");
            Element.button ~on_click:StorageReload
              ~attrs:[ ("data-testid", "storage-reload-btn") ]
              (Element.text "Reload");
          ];
        Element.box
        ...
        Element.text "Typed counter (With_codec)";
        Element.box
          ~attrs:[ ("data-testid", "codec-count") ]
          [ Element.text (string_of_int model.codec_count) ];
        Element.row ~style:button_row_style
          [
            Element.button ~on_click:CodecIncrement
              ~attrs:[ ("data-testid", "codec-increment-btn") ]
              (Element.text "+1");
            Element.button ~on_click:CodecSave
              ~attrs:[ ("data-testid", "codec-save-btn") ]
              (Element.text "Save count");
            Element.button ~on_click:CodecLoad
              ~attrs:[ ("data-testid", "codec-load-btn") ]
              (Element.text "Load count");
            Element.button ~on_click:CodecCorrupt
              ~attrs:[ ("data-testid", "codec-corrupt-btn") ]
              (Element.text "Corrupt");
          ];
        Element.box
          ~attrs:[ ("data-testid", "codec-result") ]
          [
            Element.text
              (match model.codec_state with
              | CodecIdle -> "No codec operation yet"
              | CodecSaved -> "Saved"
              | CodecLoaded n -> Printf.sprintf "Loaded %d" n
              | CodecLoadedNone -> "Not found"
              | CodecCorrupted -> "Corrupted (wrote a non-integer)"
              | CodecDecodeError msg -> "Decode error: " ^ msg
              | CodecBackendError e -> "Error: " ^ e);
          ];
      ]
