...
type msg =
  ...
  | TabPressed of string
      (** Messages for the modal demo. [TabPressed key] carries the pressed key
          ([Tab] or [Shift+Tab]); [update] resolves the next focus from it. *)
...
(** Update function.
    ...
    - [TabPressed key] resolves the next focusable element from the current
      [focused], updates [focused], and issues [Cmd.focus]. *)
...
(** Subscriptions. Wires [Modal.subscriptions] for Escape and [Sub.on_keydown]
    for Tab/Shift+Tab focus trapping when open. *)
